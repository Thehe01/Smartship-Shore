...
import java.util.concurrent.TimeUnit;import lombok.RequiredArgsConstructor;
...
public class MqttAckPublisher {
   ...
   * {@code false} on any failure, timeout, or disabled configuration.
   */
  public boolean publishAck(String mmsi, String msgId, String seq) {
    ...
    try {
      ...
      message.setQos(1);
      // MqttClient.publish is blocking with no timeout of its own: time-box it so
      // one slow broker cannot stall the serial ingest callback. A timed-out call
      // may still land late as a duplicate ACK — harmless by design (Edge matches
      // on msg_id and resends are idempotent downstream).
      ...
      try {
        ...
        }).get(mqtt.getAckTimeoutMs(), TimeUnit.MILLISECONDS);
